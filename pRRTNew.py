...
class RRT:
    ...
    def planning(self, animation, ax):
        ...
        for i in range(self.max_iter):
            print(i)
            rnd_node = self.get_random_sample()
    ...
    def get_random_sample(self):
        ...
        rnd = self.Node(return_x, return_y)
        return rnd
...
def main(sx,sy,gx,gy,obstacle_list,robot_radius,i):
    ...
    dd = 0.1
    dim = 10
    x = np.arange(-dim-dd,dim+dd,dd)
    y = np.arange(-dim-dd,dim+dd,dd)
    xx, yy = np.meshgrid(x,y)
    mu = [[gx, gy]]
    sigma = [[1,1]]
    for obs in range(len(obstacle_list)):
        mu = np.vstack([mu,[[obstacle_list[obs][0],obstacle_list[obs][1]]]])
        # sigma = np.vstack([sigma,[[0.1,0.1]]])
        sigma = np.vstack([sigma,[[obstacle_list[obs][2],obstacle_list[obs][2]]]])
    ...
    if path is None:
        ...
        if show_animation:
            ...
            plt.savefig(f'pRRT/Outdoor_0_05/prrt_g{i+1}.png')
            plt.show()
        ...
        traj_df.to_csv(f'pRRT/Outdoor_0_05/prrt_trajectory{i+1}.csv', index=False)
